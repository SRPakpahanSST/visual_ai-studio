...
st.set_page_config(page_title="VisualAI Studio", page_icon="🎨", layout="wide")
...
with st.sidebar:
    ...
    model_id = st.selectbox("Model", ["runwayml/stable-diffusion-v1-5", "stabilityai/stable-diffusion-2-1-base"], index=0)
    scheduler_name = st.selectbox("Scheduler", ["DPM++", "Euler A", "DDIM"], index=0)
...
with tab1:
    ...
    with col1:
        prompt_txt = st.text_area("Prompt", value="An astronaut standing on the moon surface, Earth visible in the sky, digital art 2D illustration style, flat colors, vector art, cartoon style", height=100)
        neg_txt = st.text_area("Negative Prompt", value="photorealistic, realistic, photograph, 3d render, messy, blurry, low quality, bad art, ugly, sketch, grainy, unfinished, chromatic aberration", height=80)
    with col2:
        ...
        if mode == "Simple":
            ...
            steps = st.slider("Inference Steps", 10, 100, 35)
    ...
    if generate_btn:
        with st.spinner("Menghasilkan gambar..."):
            if mode == "Simple":
                img = generate_simple_image(prompt_txt, neg_txt, steps, guidance, seed, model_id)
            else:
                img = generate_advanced_image(prompt_txt, neg_txt, steps, guidance, seed, model_id)
            st.image(img, caption=f"Seed: {seed}, Steps: {steps}, Guidance: {guidance}")
...
with tab4:
    ...
    steps_batch = st.slider("Steps", 10, 50, 30)
    guidance_batch = st.slider("Guidance", 1.0, 15.0, 7.5)
    if st.button("📊 Generate 4 Gambar"):
        with st.spinner("Menghasilkan 4 gambar..."):
            results = batch_inference(prompt_batch, neg_batch, 4, seed_base, steps_batch, guidance_batch)
